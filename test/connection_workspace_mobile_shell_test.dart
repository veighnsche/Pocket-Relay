...
void main() {
  ...
  testWidgets('tapping a history row opens read-only conversation detail', (
    tester,
  ) async {
    final clientsById = _buildClientsById('conn_primary', 'conn_secondary');
    final controller = _buildWorkspaceController(clientsById: clientsById);
        ...
        'session_a': const CodexWorkspaceConversationDetail(
          ...
          entries: <CodexWorkspaceConversationDetailEntry>[
            CodexWorkspaceConversationDetailEntry(
              ...
              title: 'User',
              body: 'Open the transport code',
              timestamp: null,
            ),
            CodexWorkspaceConversationDetailEntry(
              kind: CodexWorkspaceConversationDetailEntryKind.agentMessage,
              title: 'Codex Commentary',
              body: 'I’m tracing the reducer first.',
              timestamp: null,
            ),
          ],
        ),
    ...
    expect(find.text('Open the transport code'), findsOneWidget);
    expect(find.text('I’m tracing the reducer first.'), findsOneWidget);
