...
void main() {
  test('ssh-forwarded websocket transport reaches an existing websocket server', () async {
    ...
    expect(
      receivedFrames,
      <String>['{"jsonrpc":"2.0","method":"client/ping"}'],
    );
    expect(events.whereType<CodexAppServerSshAuthenticatedEvent>(), hasLength(1));
    expect(
      events.whereType<CodexAppServerSshPortForwardStartedEvent>().single,
      isA<CodexAppServerSshPortForwardStartedEvent>()
          .having((event) => event.remotePort, 'remotePort', server.port),
    );
    expect(bootstrapClient.forwardCalls, 1);
  });

  test('local SSH forward emits a distinct port-forward failure event', () async {
    final events = <CodexAppServerEvent>[];
    final bootstrapClient = _FakeSshBootstrapClient(
      onForwardLocal: (remoteHost, remotePort) async {
    ...
    expect(events.whereType<CodexAppServerSshPortForwardStartedEvent>(), hasLength(1));
    expect(
      events.whereType<CodexAppServerSshPortForwardFailedEvent>().single.message,
...
final class _FakeSshBootstrapClient implements CodexSshBootstrapClient {
  ...
  final Future<CodexSshForwardChannel> Function(String remoteHost, int remotePort)
  onForwardLocal;
